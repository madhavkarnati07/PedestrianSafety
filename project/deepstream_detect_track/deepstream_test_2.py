...
#
# PYDS - Nvidia Deep stream is the foundational algoritham
# Source code template is from Nvidia Github
# Process recorded video files, as these are video data we need streaming architecture. 
# Line Number 300 pipe line logic for streaming data 
# From Line 180, main function from reading file to process pipeline
# line 200 reads inout file. 
...
# Line Number 285 main program logic 



#Google Video Streamer Library for 
import gi
gi.require_version('Gst', '1.0')
# Google Streamer - video streaming library for input and out put
from gi.repository import GLib, Gst
#For validating architecture
from common.is_aarch_64 import is_aarch64
from common.bus_call import bus_call

#Nvidia Deep stream - streaming with deep learning library - Mian library for object detection 
import pyds

#Assigning labels for objects - Labeling 
...
def main(args):
  ...
  # Main pipeline for the program           
    ...
    pipeline.add(source)        # Input data video 
    pipeline.add(h264parser)    # encoding video data into H264  
    pipeline.add(decoder)       # H264 video decoder
    pipeline.add(streammux)     # Streaming data to the model 
    pipeline.add(pgie)          # Primary object detector - Algoritham
    pipeline.add(tracker)       # Track the object which is detected 
    pipeline.add(sgie1)         # Sample not used 
    pipeline.add(sgie2)
    pipeline.add(nvvidconv)     # Nvidia Video convertor 
    pipeline.add(nvosd)         # Make processed data to display
    pipeline.add(sink)          # Dummmy 

    # we link the elements together
    # file-source -> h264-parser -> nvh264-decoder ->
    # nvinfer -> nvvidconv -> nvosd -> video-renderer

    # link is to connect pipelines
